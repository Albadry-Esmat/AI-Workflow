...
- Validation and GATE-003 approval evidence are mandatory.
- Plan, review, confidence, and decision artifacts must describe one pipeline run.
- The runtime needs RFC 8785 canonical hashing and append-only storage.

## Execution Logic

Perform the documented freeze steps in order. Any failed precondition or schema check terminates before storage.

## Security

Never place secrets in the snapshot, verify approval from gate evidence, and reject existing artifact paths rather than overwriting them.

## Token Optimization

Pass immutable artifact identifiers and hashes downstream rather than repeatedly embedding the complete snapshot.

## Quality Check

Validate preconditions, hashes, expiry, required fields, and `implementation-contract.schema.json`; verify stored and returned digests agree.

## Failure Handling

Return the documented structured error without writing a partial contract. Retried storage must use a new path and preserve any prior artifact.

## Human-in-the-Loop

GATE-003 approval is non-bypassable; no contract may be frozen without named approval evidence.

## Skill Composition

Consumes validation, confidence, review, and decision artifacts; produces an immutable contract for implementation and change requests.
