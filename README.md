...
| `aiw validate-runtime-certification [file]` | Validate sanitized per-runtime certification evidence and promotion state |
| `aiw runtime-watch [--json] [--runtime <id>] [--strict]` | Inspect runtime versions and host verification status without starting sessions |
| `aiw release-status [--json] [--output <path>] [--strict]` | Generate a sanitized repository/live-gate handoff report |
| `aiw validate-release-approval [file]` | Validate scope-specific release decision, owner, blocker, review, and sanitized evidence requirements |
...
| `aiw install-projections --source DIR --target DIR` | Dry-run by default; install only with explicit `--confirm` and conflict review |
| `aiw website-check` | Fail when authoritative sources and `website/data/` are out of sync |
