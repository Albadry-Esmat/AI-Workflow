...
def main() -> int:
    ...
    if len(cases) != 15:
        failures.append(f"expected 15 cases, found {len(cases)}")
    if len([case for case in cases if case.get("evaluation_class") == "golden"]) != 10:
        failures.append("expected 10 golden cases")
    if len([case for case in cases if case.get("evaluation_class") == "adversarial"]) != 5:
        failures.append("expected 5 adversarial cases")
    if index.get("case_count") != len(cases) or index.get("golden_count") != 10 or index.get("adversarial_count") != 5:
