...
Then run the runtime’s version-only discovery command through its adapter. Do not pass live prompts or write requests until version, profile, budget, project classification, and approval boundaries are confirmed. If the runtime cannot expose a required operation, classify it as degraded or protocol-only.
